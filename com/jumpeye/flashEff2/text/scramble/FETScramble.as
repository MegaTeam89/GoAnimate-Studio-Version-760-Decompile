package com.jumpeye.flashEff2.text.scramble
{
   ...
   public class FETScramble extends IFlashEffText
   {
      ...
      public function FETScramble(param1:JUIComponent = null)
      {
         super();
      ...
      protected function scaleTween(param1:Object, param2:Number) : *
      {
         ...
         param1.clip.scaleX = 1 + Math.abs(Math.abs(4 * param2 - 2) - 2) * param1.scale;
         param1.clip.scaleY = 1 + Math.abs(Math.abs(4 * param2 - 2) - 2) * param1.scale;
      }
      ...
      protected function startTransition(param1:Boolean = true) : void
      {
         ...
         while(_loc6_ < _loc3_)
         {
            ...
            while(_loc7_ < _loc11_)
            {
               ...
               obj = _loc12_;
               _loc12_.scale = Math.random() * Number(this._maxScale);
               _loc12_.height = this._animationTarget.height;
               _loc12_.width = this._animationTarget.width;
               _loc12_.endY = this._animationTarget.y - this._animationTarget.height / 2 * _loc12_.scale;
               _loc12_.endX = this._animationTarget.x - this._animationTarget.width / 2 * _loc12_.scale;
               _loc12_.startscalex = this._animationTarget.scaleX;
               _loc12_.finalscalex = owner.maxScale;
               _loc12_.startscaley = this._animationTarget.scaleY;
               _loc12_.finalscaley = owner.maxScale;
               owner.tweensFinished = 0;
               _loc12_.clip.alpha = Math.abs(1 - _loc9_);
               _loc12_.startx = this._animationTarget.x;
               _loc12_.starty = this._animationTarget.y;
               _loc12_.finalx = this._animationTarget.x + 0;
               _loc12_.finaly = this._animationTarget.y + 0;
               _loc12_.prop = _loc9_;
