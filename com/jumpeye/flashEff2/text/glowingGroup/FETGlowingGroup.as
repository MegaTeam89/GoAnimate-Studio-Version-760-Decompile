package com.jumpeye.flashEff2.text.glowingGroup
{
   ...
   public class FETGlowingGroup extends IFlashEffText
   {
      ...
      private var _maxBlur:Number = 3;
      
      protected var owner:Object;
      
      private var _groupDuration:Number = 1.2;
      
      private var outTime:Number = 1;
      
      private var _preset:Number = 1;
      
      public function FETGlowingGroup(param1:JUIComponent = null)
      {
         super();
      ...
      {
         var _loc3_:Number = Math.abs(Math.abs(2 * param1.pos - 1) - 1) * this.glowAmount * 2;
         glowEffect(param1.clip,this.glowColor,this.maxBlur,this.maxBlur,_loc3_);
      }
      ...
      public function set maxBlur(param1:Number) : void
      {
         ...
         this._maxBlur = Number(Number(param1)) || Number(0);
      }
      ...
      protected function startTransition(param1:Boolean = true) : void
      {
         ...
         if(_loc8_ != 2 || _loc8_ != 3)
         {
            ...
            _loc5_ = JFERandomPreset.getPresetsArray(1,_loc2_,groupSize);
         }
         if(_loc8_ == 2)
         {
            _loc9_ = "words";
         ...
         if(_loc8_ == 3)
         {
            ...
            this.component.drawTextTable(_loc9_,this.partialGroup,this.partialPercent,this.selectedStrings,this.partialStart,this.partialBlurAmount);
            _loc2_ = this.component.absMatrix;
            _loc5_ = JFERandomPreset.getPresetsArray(9,_loc2_);
         }
         _loc2_ = this.component.absMatrix;
         ...
         {
            owner.delay = Math.max((this.tweenDuration - this.groupDuration) / (owner.maxItems / groupSize - 1),0);
         }
         if(_loc9_ == "lines")
         {
         ...
         while(_loc6_ < _loc3_)
         {
            ...
            {
               (_loc13_ = owner.elements[_loc6_][_loc7_] = {}).clip = _loc2_[_loc6_][_loc7_];
               _loc13_.clip.alpha = Math.abs(1 - _loc10_);
               checkTransitionType(param1,_loc13_);
               TweenLite.to(_loc13_,outTime * this._groupDuration,{
                  "pos":prop,
                  "delay":_loc4_[_loc6_][_loc7_] * owner.delay,
                  "ease":this.easeFunc,
                  "onUpdate":motionChange,
                  "onUpdateParams":[_loc13_,_loc13_.clip],
                  "onComplete":motionFinish,
                  "onCompleteParams":[_loc13_,_loc13_.clip]
               });
               TweenLite.to(_loc13_.clip,outTime * this._groupDuration,{
                  "alpha":Number(param1),
      ...
      private function checkTransitionType(param1:Boolean, param2:Object) : *
      {
         ...
         {
            param2.pos = 1;
            newDelay = this._groupDuration - outTime * this._groupDuration;
         }
         else
         {
            param2.pos = 0;
