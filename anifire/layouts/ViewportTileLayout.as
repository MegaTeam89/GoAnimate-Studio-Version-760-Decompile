package anifire.layouts
{
   ...
   public class ViewportTileLayout extends LayoutBase
   {
       
      
      private var _parentViewport:GroupBase;
      ...
      public function ViewportTileLayout()
      {
         super();
      }
      
      public function get parentViewport() : GroupBase
      {
         return this._parentViewport;
      }
      
      public function set parentViewport(param1:GroupBase) : void
      {
         if(param1 == this._parentViewport)
         {
            return;
         }
         this._parentViewport = param1;
      ...
      private function calculateColumnAndRowCount(param1:Number, param2:Number, param3:int) : void
      {
         ...
         var _loc17_:int = 0;
         var _loc18_:int = 0;
         this._columnCount = this._rowCount = -1;
         if(-1 != this._requestedColumnCount || -1 != this._requestedRowCount)
         {
         ...
         else
         {
            ...
            _loc10_ = _loc6_ != 0 ? Number((_loc7_ + _loc9_) / (2 * _loc6_)) : Number(param3);
      ...
      private function justifyByGapSize(param1:Number, param2:Number, param3:Number, param4:int) : Number
      {
         ...
         if(_loc5_ == 1)
         {
            return param4 > 1 ? Number(Math.max(param3,param1 - param2)) : Number(param3);
         }
      ...
      private function updateVirtualTileSize(param1:ILayoutElement) : void
      {
         ...
         this._tileWidthCached = !!isNaN(this._tileWidthCached) ? Number(_loc2_) : Number(Math.max(_loc2_,this._tileWidthCached));
         this._tileHeightCached = !!isNaN(this._tileHeightCached) ? Number(_loc3_) : Number(Math.max(_loc3_,this._tileHeightCached));
      }
      ...
      {
         var _loc6_:ILayoutElement = null;
         this._columnWidth = this._tileWidthCached;
         this._rowHeight = this._tileHeightCached;
         if(!isNaN(this._columnWidth) && !isNaN(this._rowHeight))
         {
      ...
      private function calculateDisplayParameters(param1:int, param2:int) : void
      {
         ...
         if(useVirtualLayout)
         {
            ...
            {
               _loc15_ = target.getRect(this.parentViewport);
               _loc16_ = target.localToGlobal(new Point(target.x,target.y));
               _loc17_ = this.parentViewport.globalToLocal(_loc16_);
               _loc5_ = Math.max(this.parentViewport.horizontalScrollPosition - _loc15_.x,0);
               _loc6_ = Math.max(this.parentViewport.verticalScrollPosition - _loc15_.y,0);
               _loc7_ = Math.min(this.parentViewport.horizontalScrollPosition + this.parentViewport.width - _loc17_.x,target.width);
               _loc8_ = Math.min(this.parentViewport.verticalScrollPosition + this.parentViewport.height - _loc17_.y,target.height);
            }
            _loc9_ = this._columnWidth + this._horizontalGap;
            _loc10_ = this._rowHeight + this._verticalGap;
            _loc11_ = Math.max(0,Math.floor(_loc5_ / _loc9_));
            _loc12_ = Math.max(0,Math.floor(_loc6_ / _loc10_) - 1);
            _loc13_ = Math.min(this._columnCount - 1,Math.floor(_loc7_ / _loc9_));
            _loc14_ = Math.min(this._rowCount - 1,Math.floor(_loc8_ / _loc10_) + 1);
            if(this.orientation == TileOrientation.ROWS)
            {
               this.visibleStartIndex = _loc12_ * this._columnCount;
               this.visibleEndIndex = Math.min(_loc4_ - 1,_loc14_ * this._columnCount + _loc13_);
               this.visibleStartY = _loc12_ * _loc10_ + this.paddingTop;
            }
            else
            {
               this.visibleStartIndex = _loc11_ * this._rowCount;
               this.visibleEndIndex = Math.min(_loc4_ - 1,_loc13_ * this._rowCount + _loc14_);
               this.visibleStartX = _loc11_ * _loc9_ + this.paddingLeft;
            }
         }
      }
      ...
      override public function measure() : void
      {
         ...
         var _loc4_:int = this._verticalGap;
         var _loc5_:int = this._columnWidth;
         ...
         var _loc8_:int = this._requestedColumnCount != -1 ? int(Math.max(1,this._requestedColumnCount)) : int(this._columnCount);
         var _loc9_:int = this._requestedRowCount != -1 ? int(Math.max(1,this._requestedRowCount)) : int(this._rowCount);
      ...
      override public function getNavigationDestinationIndex(param1:int, param2:uint, param3:Boolean) : int
      {
         ...
         switch(param2)
         {
            case NavigationUnit.LEFT:
               ...
               else if(param3 && _loc9_ == 0 && _loc5_ && _loc8_ == 0)
               {
            ...
            case NavigationUnit.RIGHT:
               ...
               else if(param3 && _loc9_ == this.columnCount - 1 && _loc5_ && _loc8_ == this.rowCount - 1)
               {
                  _loc9_ = 0;
                  _loc8_ = 0;
               }
            ...
            case NavigationUnit.UP:
               ...
               else if(param3 && _loc8_ == 0 && !_loc5_ && _loc9_ == 0)
               {
                  _loc9_ = this.columnCount - 1;
                  _loc8_ = this.rowCount - 1;
               }
            ...
            case NavigationUnit.DOWN:
               ...
               else if(param3 && _loc8_ == this.rowCount - 1 && !_loc5_ && _loc9_ == this.columnCount - 1)
               {
                  _loc9_ = 0;
                  _loc8_ = 0;
               }
            ...
            case NavigationUnit.PAGE_DOWN:
               ...
               if(_loc5_)
               {
                  ...
                  else if(_loc11_ <= _loc6_ && _loc6_ < _loc12_)
                  {
                     _loc8_ = _loc12_;
                  }
                  else
               ...
               else
               {
                  ...
                  else if(_loc13_ <= _loc7_ && _loc7_ < _loc14_)
                  {
                     _loc9_ = _loc14_;
                  }
                  else
      ...
      private function calculateDropCellIndex(param1:Number, param2:Number) : Array
      {
         ...
         var _loc9_:*;
         if(_loc9_ = this.orientation == TileOrientation.ROWS)
         {
      ...
      override protected function calculateDropIndicatorBounds(param1:DropLocation) : Rectangle
      {
         var _loc7_:Number = NaN;
         var _loc8_:Number = NaN;
         var _loc9_:IVisualElement = null;
         var _loc10_:Number = NaN;
         ...
         var _loc11_:Number = dropIndicator.width;
         var _loc12_:Number = dropIndicator.height;
         if(_loc5_)
         {
            _loc10_ = 0 < this._horizontalGap ? Number(this._horizontalGap) : Number(0);
            ...
            _loc7_ = Math.max(-1,Math.min(target.contentWidth - _loc11_ + 1,_loc7_));
            _loc8_ = _loc3_ * (this._rowHeight + this._verticalGap) + this.paddingTop;
         }
         else
         {
            _loc10_ = 0 < this._verticalGap ? Number(this._verticalGap) : Number(0);
            ...
            _loc8_ = Math.max(-1,Math.min(target.contentHeight - _loc12_ + 1,_loc8_));
         }
      ...
      override mx_internal function getElementNearestScrollPosition(param1:Point, param2:String = "center") : int
      {
         ...
         {
            case "topLeft":
               if(_loc13_ == _loc10_ && _loc5_ < _loc7_)
               {
                  _loc5_++;
               }
               else if(_loc13_ == _loc11_ && _loc4_ < _loc6_)
               {
                  _loc4_++;
               }
               else if(_loc13_ == _loc12_ && _loc4_ < _loc6_ && _loc5_ < _loc7_)
               {
                  _loc4_++;
                  _loc5_++;
               }
               break;
            case "topRight":
               if(_loc13_ == _loc9_ && _loc5_ > 0)
               {
                  _loc5_--;
               }
               else if(_loc13_ == _loc11_ && _loc5_ > 0 && _loc4_ < _loc6_)
               {
                  _loc5_--;
                  _loc4_++;
               }
               else if(_loc13_ == _loc12_ && _loc4_ < _loc6_)
               {
                  _loc4_++;
               }
               break;
            case "bottomLeft":
               if(_loc13_ == _loc9_ && _loc4_ > 0)
               {
                  _loc4_--;
               }
               else if(_loc13_ == _loc10_ && _loc4_ > 0 && _loc5_ < _loc7_)
               {
                  _loc4_--;
                  _loc5_++;
               }
               else if(_loc13_ == _loc12_ && _loc5_ < _loc7_)
               {
                  _loc5_++;
               }
               break;
            case "bottomRight":
               if(_loc13_ == _loc9_ && _loc5_ > 0 && _loc4_ > 0)
               {
                  _loc5_--;
                  _loc4_--;
               }
               else if(_loc13_ == _loc10_ && _loc4_ > 0)
               {
                  _loc4_--;
               }
               else if(_loc13_ == _loc11_ && _loc5_ > 0)
               {
                  _loc5_--;
               }
               break;
