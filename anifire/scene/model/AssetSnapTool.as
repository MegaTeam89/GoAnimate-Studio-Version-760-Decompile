package anifire.scene.model
{
   ...
   public class AssetSnapTool
   {
      ...
      public function updateReference(param1:SceneEditor) : void
      {
         ...
         var _loc3_:AssetViewCollection = this._sceneEditor.controller.selection;
         if(this._snapCameraEnabled)
         {
            if(this._sceneEditor.sceneModel is AnimeScene)
            {
               if(_loc4_ = AnimeScene(this._sceneEditor.sceneModel).camera)
               {
         ...
         {
            if(_loc5_ = this._sceneEditor.view.view.iterator("asset"))
            {
               while(_loc5_.hasNext)
               {
                  if((_loc6_ = _loc5_.next as DisplayObject) && !_loc3_.isInCollection(_loc6_ as IAssetView))
                  {
