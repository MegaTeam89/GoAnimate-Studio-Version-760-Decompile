package anifire.cc.view
{
   ...
   public class CCHeadView extends CcHeadComponent
   {
      ...
      private function createFaceDecorationContainers() : void
      {
         ...
         var _loc2_:* = null;
         var _loc3_:Vector.<CCBodyComponentModel> = null;
         var _loc4_:int = 0;
         ...
         if(this._myActionModel)
         {
            ...
            if(_loc1_)
            {
               for(_loc2_ in this._myActionModel.components)
               {
                  ...
                  {
                     _loc3_ = this._myActionModel.getComponentByType(_loc2_) as Vector.<CCBodyComponentModel>;
                     if(_loc3_)
                     {
                        _loc4_ = 0;
                        while(_loc4_ < _loc3_.length)
                        {
                           if((_loc5_ = _loc3_[_loc4_] as CCBodyComponentModel).id)
                           {
                              (_loc6_ = new Sprite()).name = _loc5_.id + CcLibConstant.MC_NAME_EXT;
      ...
      private function createComponentFromCam(param1:String, param2:String = "") : CcComponent
      {
         ...
         if(this._useImageLibrary)
         {
            if(this._myActionModel)
            {
               ...
               {
                  _loc5_ = (_loc4_ = this._myActionModel.getComponentByType(param1))[param2].path;
               }
               else
               {
               ...
               _loc6_ = 0;
               if((_loc6_ = CcImageLibrary.library.requestImage(_loc5_,this._sceneId,_loc3_)) > 0)
               {
                  return null;
               }
            }
      ...
      private function borrowAllComponents(param1:Boolean = true) : void
      {
         ...
         if(this._useImageLibrary)
         {
            if(this._myActionModel)
            {
               ...
               {
                  _loc3_ = null;
                  if(CcLibConstant.ALL_HEAD_COMPONENT_TYPES.indexOf(_loc4_ = String(_loc2_)) >= 0)
                  {
                     if(CcLibConstant.ALL_MULTIPLE_COMPONENT_TYPES.indexOf(_loc2_) > -1)
                     {
      ...
      private function borrowComponentByCam(param1:String, param2:Boolean, param3:String) : CcComponent
      {
         ...
         if(CcLibConstant.ALL_MULTIPLE_COMPONENT_TYPES.indexOf(param1) > -1)
         {
            _loc6_ = (_loc7_ = this._myActionModel.getComponentByType(param1))[param3].path;
         }
         else
         {
         ...
         if(_loc8_ = CcImageLibrary.library.borrowImage(_loc6_,_loc9_,this._sceneId))
         {
            this._assetImageIdArray.push(_loc6_,_loc8_.imageId);
            if(_loc4_ = _loc8_.image as CcComponent)
            {
