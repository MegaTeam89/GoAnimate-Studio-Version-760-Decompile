   ...
   public class CcHeadComponent extends MovieClip
   {
      ...
      private function createAllComponents(param1:Boolean = false) : void
      {
         ...
         if(this._charXml)
         {
            ...
            {
               _loc3_ = null;
               if(CcLibConstant.ALL_HEAD_COMPONENT_TYPES.indexOf(_loc6_ = String(_loc2_.@type)) >= 0)
               {
      ...
      private function borrowComponent(param1:XML, param2:String, param3:Boolean) : CcComponent
      {
         ...
         if(_loc7_ = CcImageLibrary.library.borrowImage(_loc6_,_loc8_,this._sceneId))
         {
            this._assetImageIdArray.push(_loc6_,_loc7_.imageId);
            if(_loc4_ = _loc7_.image as CcComponent)
            {
               (_loc9_ = CcComponentModel.createModelByType(param1.@type)).initByXml(param1);
               _loc4_.reset(_loc9_);
            }
         }
         return _loc4_;
      ...
      private function createComponentFromXml(param1:XML, param2:String) : CcComponent
      {
         ...
         if(this._useImageLibrary)
         {
            if(param1)
            {
               ...
               _loc5_ = 0;
               if((_loc5_ = CcImageLibrary.library.requestImage(_loc4_,this._sceneId,_loc3_)) > 0)
               {
                  return null;
               }
            }
         ...
         if(_loc3_)
         {
            (_loc6_ = CcComponentModel.createModelByType(param1.@type)).initByXml(param1);
            _loc3_.init(_loc6_);
         }
         return _loc3_;
      }
      ...
      private function borrowAllComponents(param1:Boolean = true) : void
      {
         ...
         if(this._useImageLibrary)
         {
            if(this._charXml)
            {
               ...
               {
                  _loc3_ = null;
                  if(CcLibConstant.ALL_HEAD_COMPONENT_TYPES.indexOf(_loc5_ = String(_loc2_.@type)) >= 0)
                  {
                     if(_loc4_ == CcLibConstant.COMPONENT_TYPE_FACIAL_DECORATION)
                     {
                        _loc5_ += _loc2_.@id;
      ...
      public function setComponent(param1:ICcComponent, param2:ByteArray) : Boolean
      {
         ...
         {
            _loc3_.init(param1);
            if((_loc4_ = param1.type) == "facedecoration")
            {
               _loc4_ = param1.id;
               _loc5_ = param1.id + CcLibConstant.MC_NAME_EXT;
               if(!(_loc6_ = UtilPlain.getInstance(this,_loc5_)))
               {
                  if(_loc7_ = this._containers.getValueByKey("facedecorationMC"))
                  {
                     (_loc8_ = new Sprite()).name = _loc5_;
