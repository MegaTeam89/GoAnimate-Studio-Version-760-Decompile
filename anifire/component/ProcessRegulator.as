package anifire.component
{
   ...
   public class ProcessRegulator extends EventDispatcher implements IRegulatedProcess
   {
      ...
      private function startNext() : void
      {
         ...
         {
            ++this._step;
            ProcessData(this._processes[this._step - 1]).process.addEventListener(ProcessData(this._processes[this._step - 1]).eventType,this.onProcessComplete);
            ProcessData(this._processes[this._step - 1]).process.startProcess(this._oneByOne,this._interval);
         }
      }
