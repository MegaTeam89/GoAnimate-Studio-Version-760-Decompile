   ...
   public class Bubble extends Sprite
   {
      ...
      public function showEditMode() : Object
      {
         ...
         if(!this._editEnable)
         {
            ...
            _loc1_ = Class(getDefinitionByName("anifire.font.FontChooser"));
      ...
      public function serialize() : XML
      {
         ...
         if(_loc2_ == this._promptText)
         {
            this._text = this._backupText = "";
         }
         if(this._text != this._backupText)
         {
            _loc1_ = <bubble x="{Util.roundNum(this._x)}" y="{Util.roundNum(this._y)}" w="{Util.roundNum(this._width)}" h="{Util.roundNum(this._height)}" rotate='0' type="{this._type}" hasTail="{!!this._tailEnable ? "1" : "0"}" colorStyle="{this._colorStyle}" autoTextResize="{this._autoTextResize}">
												<body rgb="{this._fillRgb}" shadow="{!!this._shadow ? "1" : "0"}" bgAlpha="{this._fillAlpha}" linergb="{this._lineRgb}" tailx="{Util.roundNum(this._tailx)}" taily="{Util.roundNum(this._taily)}"/>
         ...
         if(_loc2_ == this._promptText)
         {
            this._text = this._backupText = _loc2_;
         }
         return _loc1_;
      }
      ...
      public function addedToStageHandler(param1:Event, param2:String = null) : void
      {
         ...
         {
            if((_loc5_ = param1.target as AddedToStage) != null && _loc5_.hasEventListener("AddedToStageEv"))
            {
               _loc5_.removeEventListener("AddedToStageEv",this.addedToStageHandler);
            }
         }
         ...
         if(_loc4_ = _loc3_.font)
         {
            ...
            {
               FontManager.getFontManager().loadFont(_loc4_,this.addedToStageHandler);
            }
            else
            {
               this.redraw();
