package anifire.core
{
   ...
   public class CCManager extends EventDispatcher
   {
      ...
      protected function changeSkinColor(param1:DisplayObjectContainer) : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            if((_loc5_ = (_loc4_ = param1.getChildAt(_loc3_)).name) && _loc5_.indexOf("theColor_") != -1)
            {
               _loc6_ = _loc5_.substr(9);
               if((_loc7_ = this.getColorByName(_loc6_)) != uint.MAX_VALUE)
               {
                  (_loc8_ = new ColorTransform()).color = _loc7_;
