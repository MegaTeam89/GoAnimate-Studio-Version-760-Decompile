package anifire.core.sound
{
   ...
   public class SoundCollectionController extends EventDispatcher
   {
      ...
      {
         this._permutations = UtilMath.permutate([0,1,2,3]);
         super(param2);
         this._sounds = param1;
      }
      ...
      {
         this.wrapEnabled = !this._wrapEnabled;
      }
      ...
      public function removeSoundById(param1:String) : void
      {
         if(this._sounds)
         {
            this._sounds.removeSoundById(param1);
         }
         if(this.wrapEnabled)
         {
            this.wrapSoundCollection();
      ...
      public function checkSoundOverlap(param1:AnimeSound) : int
      {
      ...
      public function resolveSoundOverlap(param1:AnimeSound, param2:Boolean = false) : int
      {
         var _loc3_:int = 0;
         ...
         if(this._sounds && param1)
         {
            ...
            {
               if((_loc5_ = _loc4_.next as AnimeSound) && _loc5_ != param1)
               {
      ...
      public function wrapSoundCollection(param1:Boolean = false) : int
      {
         ...
         if(this.wrapEnabled && this._sounds)
         {
            ...
            while(_loc4_.hasNext)
            {
               if(_loc5_ = _loc4_.next as AnimeSound)
               {
                  _loc5_.trackNum = 0;
      ...
      private function selectSoundsByTrack(param1:int, param2:ICollection, param3:SoundSelection) : SoundSelection
      {
         ...
         if(param2)
         {
            ...
            {
               if((_loc4_ = _loc5_.next as AnimeSound) && _loc4_.trackNum == param1)
               {
      ...
      public function refreshOrder(param1:AnimeSound = null) : void
      {
         ...
         if(this.wrapEnabled)
         {
            ...
            while(_loc5_ < 4)
            {
               this.reorderSounds(_loc2_[_loc5_] as SoundSelection);
               _loc5_++;
            }
         }
      }
      ...
      public function resolveOverlap(param1:SoundSelection) : int
      {
         ...
         if(param1)
         {
            ...
            {
               if(_loc9_ = _loc8_.next as AnimeSound)
               {
      ...
      public function getMaxLeftOffset(param1:ICollection) : int
      {
         ...
         if(param1)
         {
            ...
            while(_loc3_.hasNext)
            {
               ...
               {
                  if((_loc5_ = StudioSoundManager.getLastSoundBeforeFrameOnTrack(this._sounds,_loc4_.startFrame,_loc4_.trackNum,_loc4_)) && param1 is SoundSelection && SoundSelection(param1).isSelected(_loc5_) == false)
                  {
      ...
      public function get collection() : SoundCollection
      {
         return this._sounds;
      }
      ...
      {
         var _loc1_:AlertPopUp = new AlertPopUp();
         _loc1_.title = UtilDict.toDisplay("go","Sound Clip Overlapping");
         _loc1_.message = UtilDict.toDisplay("go","Following your latest change, some sound clips are overlapping in your timeline. This happened because you cannot have more than 4 sounds clips playing at the same time.");
         _loc1_.confirmText = UtilDict.toDisplay("go","Undo Changes");
         _loc1_.showCancelButton = false;
         _loc1_.showCloseButton = false;
         _loc1_.addEventListener(PopUpEvent.CLOSE,this.onPopUpClose);
